"""Bootstrap the information-safety venv on the trc /work volume.

Run once per trc workspace; subsequent ``migrate_pool_to_trc.py`` submissions
assume ``/work/envs/information-safety/.venv`` exists and the python it points
at is on the persistent ``/work`` volume (not in a per-container ephemeral
``$HOME``). HF model snapshots are downloaded on demand by each pool job into
``/work/.hf-cache``, which is shared across jobs.
"""
...
TRC_UV_PYTHON_INSTALL_DIR = "/work/.uv/python"
...
def build_eai_submit_argv(*, hf_token: str, git_sha: str) -> list[str]:
    ...
    container_cmd = " && ".join([
        ...
        f"export UV_PYTHON_INSTALL_DIR={TRC_UV_PYTHON_INSTALL_DIR}",

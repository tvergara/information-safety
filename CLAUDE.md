...
**NEVER dismiss test failures as "pre-existing."** If a test fails, fix it. If a dependency is missing, install it with `uv pip install` — do NOT add `try/except` skips or `TYPE_CHECKING` workarounds as a substitute for having the package installed.

...
For example, when running `python information_safety/main.py algorithm=image_classifier`.
...
flowchart TD
datamodule_config[
    <a href="#information_safety.conftest.datamodule_config">datamodule_config</a>
] -- 'datamodule=A' --> command_line_arguments
algorithm_config[
    <a href="#information_safety.conftest.algorithm_config">algorithm_config</a>
] -- 'algorithm=B' --> command_line_arguments
command_line_overrides[
    <a href="#information_safety.conftest.command_line_overrides">command_line_overrides</a>
] -- 'seed=123' --> command_line_arguments
command_line_arguments[
    <a href="#information_safety.conftest.command_line_arguments">command_line_arguments</a>
] -- load configs for 'datamodule=A algorithm=B seed=123' --> dict_config
dict_config[
    <a href="#information_safety.conftest.dict_config">dict_config</a>
] -- instantiate objects from configs --> config
config[
    <a href="#information_safety.conftest.config">config</a>
] --> datamodule & algorithm
datamodule[
    <a href="#information_safety.conftest.datamodule">datamodule</a>
] --> algorithm
algorithm[
    <a href="#information_safety.conftest.algorithm">algorithm</a>
...
from information_safety.experiment import instantiate_trainer
from information_safety.main import PROJECT_NAME, instantiate_algorithm, setup_logging
from information_safety.utils.env_vars import NUM_WORKERS, REPO_ROOTDIR
from information_safety.utils.hydra_utils import resolve_dictconfig
from information_safety.utils.testutils import (
    IN_GITHUB_CI,
    PARAM_WHEN_USED_MARK_NAME,
    default_marks_for_config_combinations,
    default_marks_for_config_name,
)
from information_safety.utils.typing_utils import is_sequence_of
...
def algorithm_config(request: pytest.FixtureRequest) -> str | None:
    ...
    included tests, for example as is done in [information_safety.algorithms.image_classifier_test][].
    """
...
def command_line_arguments(
    ...
    indirectly by most tests using the [`information_safety.utils.testutils.run_for_all_configs_of_type`][]
...
def dict_config(
    ...
    logger.info(
        ...
        + "\n"
        + "```\n"
        + ("python information_safety/main.py " + " ".join(command_line_arguments) + "\n")
        + "```\n"
    )
...
def setup_with_overrides(
    ...
    def test_something(dict_config: omegaconf.DictConfig):
        ...
        assert dict_config["algorithm"]["_target_"] == "information_safety.algorithm.image_classifier.ImageClassifier"

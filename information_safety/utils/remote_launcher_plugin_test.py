...
# Use monkeypatch.setattr(information_safety.utils.remote_launcher_plugin, ..., that_mock)
...
import information_safety.utils.remote_launcher_plugin
from information_safety.main import PROJECT_NAME, main
from information_safety.main_test import CONFIG_DIR
from information_safety.utils import remote_launcher_plugin
from information_safety.utils.env_vars import SLURM_JOB_ID
from information_safety.utils.remote_launcher_plugin import RemoteSlurmLauncher
from information_safety.utils.testutils import IN_GITHUB_CI, IN_SELF_HOSTED_GITHUB_CI
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            f"algorithm=attack_with_strategy trainer.fast_dev_run=True cluster={cluster} resources={resources}",
            marks=[
                pytest.mark.skipif(
...
@pytest.mark.parametrize(
    ...
    [
        [
            "algorithm=attack_with_strategy",
...
def test_run_remote_job(argv: list[str], monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        information_safety.utils.remote_launcher_plugin, RemoteSlurmLauncher.__name__, launcher_mock
    )

    monkeypatch.setattr(sys, "argv", ["information_safety/main.py"] + argv)
